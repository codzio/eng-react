...
const master = require('../../helpers/master_helper');

const isCsrfValid = value => {
  if (!csrf.verifyCsrf(value)) {
...
exports.dashboard = (req, res, next) => {
  res.render("admin/dashboard", {
    title: master.siteTitle('Dashboard'),
...
exports.accountSettings = async (req, res, next) => {
   ...
   try {
      ...
      const userData = await master.getData(req.session.user.email);
      
      res.render("admin/account_settings", {
        title: master.siteTitle('Account Settings'),
        currentMenu: req.path
      });
