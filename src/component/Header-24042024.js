...
function Header(props) {
    ...
    return(
        <header className= {`header__main ${isSticky ? 'sticky' : ''}`}>
			<div className={`header_inner ${ismobileNav ? 'brd' : ""}`}>
					...
					<ul className={`${ismobileNav ? 'show brd' : ""}`}>
						...
						<li className={isActive('/products')}>
							<span>
								...
								<i className={`fa-solid fa-chevron-down ${isDropdownOpen ? 'rotate' : ''}`} onClick={toggleDropdown}></i>
							</span>
												...
												<ul>													
													{data.product.map((data) => (
														...
														{data.subProduct && data.subProduct.length > 0 ? (
															<div className={`sub-dropdown inner`}>
																<ul>
						...
						<li className={isActive('/blogs','/projects','/use-cases','/career')}>
							<span>
								...
								<i className={`fa-solid fa-chevron-down ${isSecondDropdownOpen ? 'rotate' : ''}`} onClick={toggleDropdownSecond}></i>
							</span>
